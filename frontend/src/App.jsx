...
export default function App() {
  ...
  useEffect(() => {
    if (token) {
      fetchData();
      const interval = setInterval(fetchData, 8000); // 8s polling for demo
  ...
  const triggerSimulator = async () => {
    setSimulating(true);
    try {
      await fetch(`${API_BASE}/simulator/simulate-checkin`, { method: 'POST' });
      await fetchData();
    } catch (err) {
  ...
  const handleAlertAction = async (actionType) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/alerts/${selectedAlert.id}/resolve`, {
        method: 'PATCH',
        headers: {
        ...
        body: JSON.stringify({
          action: actionType,
          notes: resolveNotes || (actionType === 'ACKNOWLEDGE' ? 'Acknowledged by Supervisor' : 'Duty reassigned and incident reported.')
        })
      });
      if (res.ok) {
  ...
  const handleCalibrate = async (kioskId) => {
    try {
      const res = await fetch(`${API_BASE}/kiosks/${kioskId}/calibrate`, {
        method: 'POST',
  ...
  const exportTransactionsCSV = () => {
    ...
    const headers = ["Transaction UUID,Timestamp,Emp Code,Name,Station,Kiosk,BrAC (mg/100ml),Result,Duty,CRIS Synced\n"];
    const rows = transactions.map(t => 
      `"${t.transaction_uuid}","${t.timestamp}","${t.emp_code}","${t.emp_name}","${t.station}","${t.kiosk_code}","${t.brac_reading_mg100ml}","${t.test_result}","${t.duty_type}","${t.synced_to_cris}"`
    ).join("\n");
  ...
  // If unauthenticated: Show Indian Railways Branded Login
  if (!token || !user) {
    return (
        ...
        <div className="max-w-md w-full bg-white rounded-2xl shadow-2xl overflow-hidden border border-slate-800">
          ...
          <form onSubmit={handleLogin} className="p-8 space-y-5">
            ...
            <div className="text-xs bg-slate-50 p-3 rounded-lg border border-slate-200 text-slate-600 space-y-1">
              ...
              <p>• Admin: <code className="bg-slate-200 px-1 py-0.5 rounded">admin</code> / <code className="bg-slate-200 px-1 py-0.5 rounded">Admin@123</code></p>
              <p>• Supervisor: <code className="bg-slate-200 px-1 py-0.5 rounded">supervisor_sc</code> / <code className="bg-slate-200 px-1 py-0.5 rounded">Super@123</code></p>
            </div>
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col font-sans">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="bg-white p-5 rounded-xl border border-rose-200 shadow-sm bg-rose-50/20">
                ...
                <p className="text-xs text-rose-500 font-medium mt-3">
                  Immediate supervisor escalation active
                </p>
              </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-rose-700 mt-0.5">
                      Review immediate positive alcohol results and sensor calibration due dates.
                    </p>
                  </div>
                </div>
                <button 
                      ...
                      <tr key={tx.id} className="hover:bg-slate-50/80 transition">
                        ...
                        <td className="px-6 py-3.5 font-mono">
                          <span className={`font-bold ${tx.brac_reading_mg100ml > 0 ? 'text-rose-600 text-sm' : 'text-slate-700'}`}>
                            {tx.brac_reading_mg100ml.toFixed(1)}
                          </span>
                    ...
                    <tr key={tx.id} className="hover:bg-slate-50 transition">
                      ...
                      <td className="px-6 py-3.5 font-mono">
                        <span className={`font-bold ${tx.brac_reading_mg100ml > 0 ? 'text-rose-600' : 'text-slate-800'}`}>
                          {tx.brac_reading_mg100ml.toFixed(1)} mg/100ml
                        </span>
                      </td>
                      <td className="px-6 py-3.5">
                        <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-bold ${
        ...
        {activeTab === 'hhts' && (
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              {/* Map Placeholder with Live Coordinate Visualizer */}
      ...
      {/* ================= MODAL: TEST EVIDENCE INSPECTION ================= */}
      {selectedTx && (
